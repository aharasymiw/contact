...
import {
  inviteCallRequestSchema,
  relaySignalRequestSchema,
  respondToCallRequestSchema,
...
import { createInMemoryRateLimiter } from "./lib/rate-limit.ts";
import {
...
import { createHttpError, parseOrThrow } from "./lib/validation.ts";
...
interface PendingInviteRow extends CallRow {
  caller_id: string;
  caller_username: string;
  caller_display_name: string;
  caller_created_at: string;
}
...
app.use(createSecurityHeadersMiddleware({ isProduction: config.isProduction }));
app.use(
  createTrustedOriginMiddleware({
...
function requireCurrentUser(request: AppRequest): User {
  if (!request.currentUser) {
    throw createHttpError("You need to log in before using the live call lab.", 401);
  }

  return request.currentUser;
}

function serializeCall(row: CallRow, currentUserId: string): RemoteCall {
...
async function loadPeerUsers(currentUserId: string) {
  const result = await query<PeerUserRow>(
    `
...
async function loadPendingInvites(currentUserId: string) {
  const result = await query<PendingInviteRow>(
    `
      select
        call_sessions.id,
...
async function createBootstrapPayload(currentUser: User | null): Promise<BootstrapPayload> {
  if (!currentUser) {
    return {
      authenticated: false,
...
async function loadCallForParticipant(
  callId: string,
...
function otherParticipantId(callRow: CallRow, currentUserId: string): string {
...
app.get("/api/bootstrap", async (request: AppRequest, response: Response, next: NextFunction) => {
  try {
...
app.post(
  "/api/register",
  registerRateLimiter.middleware((request) => request.ip || "unknown"),
  async (request: AppRequest, response: Response, next: NextFunction) => {
    try {
...
app.post(
  "/api/login",
  loginRateLimiter.middleware((request) => request.ip || "unknown"),
  async (request: AppRequest, response: Response, next: NextFunction) => {
    try {
...
app.post("/api/logout", async (request: AppRequest, response: Response, next: NextFunction) => {
  try {
...
app.get(
  ...
  async (request: AppRequest, response: Response, next: NextFunction) => {
    try {
      ...
      addRealtimeConnection(currentUser.id, response);
      ...
      request.on("close", () => {
        ...
        removeRealtimeConnection(currentUser.id, response);
        broadcast("presence-update", {
          onlineUserIds: listOnlineUserIds(),
        });
      });
...
app.post(
  ...
  async (request: AppRequest, response: Response, next: NextFunction) => {
    try {
      const currentUser = requireCurrentUser(request);
      const { calleeUserId } = parseOrThrow(
        inviteCallRequestSchema,
        request.body,
        "A calleeUserId is required.",
      );

      if (calleeUserId === currentUser.id) {
        response.status(400).json({ error: "You cannot call your own account." });
        return;
      }

      const calleeResult = await query<PeerUserRow>(
        `
        select id, username, display_name, created_at
        from app_users
        where id = $1
      ...
      const result = await query<CallRow>(
        ...
        returning *
      `,
        [currentUser.id, calleeUserId],
      );

      const [callRow] = result.rows;
      const callee = serializeUser(calleeResult.rows[0]);

      await logCallEvent(callRow.id, currentUser.id, "invite-created", {
        calleeUserId,
      });
      ...
      response.status(201).json({
        call: serializeCall(callRow, currentUser.id),
        toUser: callee,
      });
    } catch (error) {
      next(error);
...
app.post(
  ...
  async (request: AppRequest, response: Response, next: NextFunction) => {
    try {
      const currentUser = requireCurrentUser(request);
      const { callId } = parseOrThrow(callParamsSchema, request.params, "A callId is required.");
      const { accept } = parseOrThrow(
        respondToCallRequestSchema,
        request.body,
        "The call response payload is invalid.",
      );
      const callRow = await loadCallForParticipant(callId, currentUser.id);
      ...
      if (callRow.callee_user_id !== currentUser.id) {
      ...
      await logCallEvent(callId, currentUser.id, "invite-responded", {
      ...
      response.json({
        accepted: Boolean(accept),
        call: serializeCall(updatedCall, currentUser.id),
      });
    } catch (error) {
      next(error);
    }
...
app.post(
  ...
  async (request: AppRequest, response: Response, next: NextFunction) => {
    try {
      const currentUser = requireCurrentUser(request);
      const { callId } = parseOrThrow(callParamsSchema, request.params, "A callId is required.");
      const { kind, payload } = parseOrThrow(
        relaySignalRequestSchema,
        request.body,
      ...
      await logCallEvent(callId, currentUser.id, `signal:${kind}`, {
        kind,
        payload,
      });

      sendToUser(otherParticipantId(callRow, currentUser.id), "call-signal", {
        callId,
        kind,
        payload,
        fromUser: currentUser,
      ...
      response.status(202).json({
        ok: true,
        relayedToUserId: otherParticipantId(callRow, currentUser.id),
      });
    } catch (error) {
      next(error);
    }
...
app.post(
  ...
  async (request: AppRequest, response: Response, next: NextFunction) => {
    try {
      ...
      await logCallEvent(callId, currentUser.id, "call-ended", {});

      sendToUser(otherParticipantId(updatedCall, currentUser.id), "call-ended", {
        call: serializeCall(updatedCall, otherParticipantId(updatedCall, currentUser.id)),
        fromUser: currentUser,
      });
